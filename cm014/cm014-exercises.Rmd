...
In R, takes the form `y ~ x1 + x2 + ... + xp` (use column names in your data frame). ~ to predict left using variables in the right side. 
...
```{r}
predict(my_lm) %>% #takes x values and fix the model to give a single y value
head()
```
...
```{r}
tidy(my_lm) # pvalue->significant or not. 
augment(my_lm, newdata = tibble(gdpPercap=c(400,500,600))) # original values and estimates to compare
glance(my_lm) #how well the model fits to the data
```